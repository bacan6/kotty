<table style='font-size:12px;color:white' width="100%">
	<?php
	...
	?>
		...
		<?php if ($kategori_member!=4 && $kategori_member!=2){?>
		<tr>
			<td width='30%'><b>Reimbursment</b></td>
			<td align='right'> <input type='number' name="poin_reimburs" style="border:0;border-bottom:solid 1px #ccc;color:black" id='poin-reimbursment' style='text-align:right;' value="<?php echo $poinValue; ?>" min="0"/> </td>
		</tr>
...
<script type="text/javascript">
	<?php if ($kategori_member==2){ ?> 
	$('.changeDiskon').prop('readonly', false);
	$('#diskon').prop('readonly', false);
	<?php } ?>
	$('#poin-reimbursment').on("change",function(){
		...
		$.ajax({
					...
					success : function(totalPurchase){
							  ...
							  if(totalPurchaseConvert > 0){
									...
									if(poinVal > max){
										$.Notification.notify('error', 'top right', 'Reimburs poin gagal', 'Inputan poin melebihi jumlah poin yang dimiliki customer');
	...
	$('.deleteMemberSelected').on("click",function(){
		var idUser = this.id;
		var urlDeleteDiscMember = "<?php echo base_url('penjualan/deleteDiscMember'); ?>";

		$.post(urlDeleteDiscMember,{idUser : idUser},function(){
			$('#data-customer').empty();
			$('#poin-value-reimburs').empty();
			viewPricePanel();
			$('#customer-form').select2("val","");
			$('.changeDiskon').prop('readonly', true);
