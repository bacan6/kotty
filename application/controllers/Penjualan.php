...
class Penjualan extends BaseController
{
	function __construct()
	{
		...
		$this->load->model(array("model1", "model_penjualan", "model_crypto"));
		$this->load->database();

		$this->isLoggedIn($this->global['idUser'], 3, 7);
	}
	function cek_BSI()
	{
	...
	function cekResi()
	{
		...
		$this->db->select(array("ap_invoice_online.ID", "ap_invoice_online.no_invoice"));
		...
		if ($cek < 1 || empty($no_online)) {
			redirect("penjualan/scanResi");
		} else {
			if (!empty($inv)) {
				redirect("penjualan/suratJalan?no_invoice=" . $inv);
			} else {
				redirect("penjualan/index?struk=" . $no_online);
			}
	...
	function scanResi()
	{
		$this->global['pageTitle'] = "SOLUSI POS - Scan Resi";
		$this->global['navigation'] = $this->model1->callNavigation();
		$this->loadViews("invoice_online/scan_resi", $this->global, null, "footer_empty");
	}
	...
	function open_verifikasi()
	{
		...
		$this->model_penjualan->insertCartApproved($data);
		$respon = array("username" => $idUser);
	...
	function cek_verifikasi()
	{
		$ip = $_SERVER['REMOTE_ADDR'];
		$idUser = $_POST['username'];
		$cek = $this->db->get_where("ap_cart_approved", array("idUser" => $idUser, "ip" => $ip, "status" => 1));
		$rows = $cek->num_rows();
		if ($rows > 0) {
			$ps = $this->model_penjualan->cekPass($idUser);
			$respon = array("status" => 1, "ps" => $ps);
		} else {
			$respon = array("status" => 0, "ps" => '');
		}
		echo json_encode($respon);
	}
	function index()
	{
		...
		if ($this->global['idUser'] == 6) {
			$data['payment_type'] = $this->db->get_where("ap_payment_type", array("id" => '5'));
		} else {
			//$this->db->order_by('name', 'asc');
			$data['payment_type'] = $this->model_penjualan->tipeBayar();
		}
		...
		$cekSetoran = $this->model1->cekSetoran($this->global['idUser']);

		if ($cekClosing < 1 && $cekSetoran < 1) {
			if ($this->global['idStore'] == 6 || $this->global['idUser'] == 57) {
				$struk = $this->input->get("struk");
				$online_inv = $this->db->get_where("ap_invoice_online", array("no_online" => $struk, "status" => 0, "id_toko" => $this->global['idStore']));
				$rows = $online_inv->num_rows();
				if ($rows > 0) {
					// do nothing
				} else {
					redirect("penjualan/scanResi");
			...
			$data['banner'] = $this->db->get_where('k_banner', array("posisi" => 'kasir'), 1);
	...
	function invoice_setorankasir()
	{
		...
		$this->db->select("ap_payment_account.*,ap_payment_type.payment_type,
			SUM(ap_invoice_number.total+ap_invoice_number.surcharge-ap_invoice_number.diskon-ap_invoice_number.diskon_free-ap_invoice_number.diskon_otomatis-ap_invoice_number.poin_value) as penjualan");
		$this->db->from("ap_invoice_number");
		$this->db->join("ap_payment_type", "ap_payment_type.id = ap_invoice_number.tipe_bayar");
		$this->db->join("ap_payment_account", "ap_invoice_number.tipe_bayar = ap_payment_account.id_payment_type and ap_invoice_number.sub_account=ap_payment_account.id_payment_account", "LEFT");
		...
		$data['voucher'] = $this->model_penjualan->voucher_value($idKasir, $tanggal);
		$data['retur'] = $this->model_penjualan->retur_value($idKasir, $tanggal) + 0;
		$data['retur_count'] = $this->model_penjualan->retur_count($idKasir, $tanggal) + 0;
	...
	function returPenjualanSQL()
	{
		...
		foreach ($decodeJSON as $dt) {
			...
			if ($qty > 0 && $inv_qty > 0) {
				...
				$this->model_penjualan->updateInvoiceItemRetur($no_invoice, $sku, $dataUpdate2);
	...
	function invoice_penjualan()
	{
		...
		$arr_kasir_online = array(9, 24, 29);
		if (in_array($this->global['idUser'], $arr_kasir_online)) {
			$this->loadViews("penjualan/body_invoice_penjualan_online", $this->global, $data, "penjualan/footerInvoicePenjualan_online");
		} else {
	...
	function sub_account()
	{
		...
		$query = $this->db->order_by('urutan', 'ASC')->get_where("ap_payment_account", array("id_payment_type" => $id));
	...
	function ajax_customer()
	{
		...
		foreach ($customer->result() as $row) {
			$data_array[] = array(
				"id" => $row->id_customer,
				"text" => $row->nama . " / " . $row->no_kartu . " / " . $row->point . "poin"
			);
		}
	...
	function ambil_nilai_diskon()
	{
		...
		$idStore = $this->global['idStore'];
		$this->db->order_by('(discount+disc_supplier)', 'DESC');
	...
	function pendingList()
	{
		...
		$data['pendingList'] = $this->db->get()->result();

		//$data['pendingList'] = $this->db->get_where("ap_cart_temp_no",array("idUser" => $idUser,"status" => 0,"tanggal like '$date%'"))->result();
	...
	{
		// hapus dulu approver yg masih ada di cart
		$ip = $_SERVER['REMOTE_ADDR'];
		$this->model_penjualan->deleteApprover($ip);
		$data['approver'] = $this->db->get("user_approver");
	...
	function getDataProduk()
	{
		...
		$sku = $_POST['sku'];
		$idStore = $this->global['idStore'];
		$id_customer = $this->model_penjualan->getIdMemberDiskon($this->global['idUser']);
		$dataProduk = $this->model_penjualan->getProdukData($sku, $idStore);

		foreach ($dataProduk as $row) {
			//$stok = ;
			if ($row->harga_member > 0 && !empty($id_customer)) {
				$arrayData[] = array(
	...
	function ambilNilaiDiskon($sku, $qty)
	{
		...
		$kategori_member = $this->model_penjualan->cekKategoriMember($id_customer) + 0;

		$bind = array('0', $kategori_member);

		$this->db->select('*');
		$this->db->from("ap_produk_discount_rules");
		$this->db->where_in('tipe', $bind);
		...
		if ($countRulesIfExist > 0) {
			foreach ($nilaiDiskon->result() as $row) {
				...
				if ($qty >= $row->qty) {
					if (strtotime(date('Y-m-d')) >= strtotime($row->date_start) && strtotime(date('Y-m-d')) <= strtotime($row->date_end)) {
						// $terjual = $this->model_penjualan->cekQuotaDiskon($sku, $row->date_start, $row->date_end, $row->JamMulai, $row->JamSelesai, $row->setJam, $row->discount, $idStore);
						$terjual = $row->quota_terpakai;
						if ($row->quota > 0 && ($terjual + $qty) < $row->quota) {
							if ($row->setHari == 1) {
								$HariID = explode(".", $row->HariID);
								if (in_array(date('w'), $HariID)) {
									if ($row->setJam == 1) {
	...
	function insertCart()
	{
		...
		// qty yg dihitung buy n get n
		$qty_get = 0;

		// Buy 1 Get 3 
		$cekNoPromo = $this->model_penjualan->cekBuy1Get3Promo($sku);
		if (!empty($cekNoPromo->no_promo)) {

			// $dataBut1Get3 = array(
			// 					"diskon" => 0
			// );
			// $this->model_penjualan->updateBuy1Get3($cekNoPromo->no_promo,$idUser,$dataBut1Get3);
			$cekBuy1Get3Qty = $this->model_penjualan->cekBuy1Get3QtyIns($cekNoPromo->no_promo, $idUser, $sku);
			if ($cekNoPromo->tipe == 'lowest') {


				$priceOn = $this->model_penjualan->cekPriceOnCartBuy1($cekNoPromo->no_promo, $idUser, $sku);
				$priceOff = $this->model_penjualan->getProdukPrice($sku, $this->global['idStore']);

				$lowestPrice = $priceOn >= $priceOff ? $priceOff : $priceOn;

				$gdiskon = (floor(($qty + $cekBuy1Get3Qty) / $cekNoPromo->qty_diskon)) * $lowestPrice;
			} else {
				$gdiskon = (floor(($qty + $cekBuy1Get3Qty) / $cekNoPromo->qty_diskon)) * $cekNoPromo->discount;
			}

			if ($gdiskon > 0)
				$qty_get = $cekNoPromo->free_item;


			$cekBuy1Get3Exists = $this->model_penjualan->cekBuy1Get3Exists($cekNoPromo->no_promo, $idUser);
			if (empty($cekBuy1Get3Exists)) {
				$dataBut1Get3 = array(
					"idUser" => $idUser,
					"no_promo" => $cekNoPromo->no_promo,
					"diskon" => $gdiskon
				);
				$this->model_penjualan->insertBuy1Get3($dataBut1Get3);
		...
		if ($cekDiskonBrand) {
			if ($cekDiskonBrand['tipe'] == 'percent') {
				$nilaiDiskon = ($cekDiskonBrand['nilai'] * $harga) * ($qty - $qty_get);
			} else
				$nilaiDiskon = 0;
		...
		if ($cekDiskon == 1) {
			...
			if ($cekTebusMurah == $sku) {
				...
				$nilaiDiskon = $this->diskonTebusMurah($sku, ($qty - $qty_get));
				$disc_tebusmurah = $nilaiDiskon;
				//if($this->global['idUser']==34) die(var_dump($nilaiDiskon)."$sku,($qty-$qty_get)");



				//echo $nilaiDiskon;
				if ($nilaiDiskon == 0) {
					$nilai = $this->ambilNilaiDiskon($sku, ($qty - $qty_get));

					if (is_object($nilai)) {
						$nilaiDiskon = ($nilai->discount + $nilai->disc_supplier) * ($qty - $qty_get);
						$disc_supplier = $nilai->disc_supplier * ($qty - $qty_get);
						$no_promo = $nilai->no_promo;
					} else
						$nilaiDiskon = $nilai * ($qty - $qty_get);
				}
	...
	function insertCartPending()
	{
		...
		if (!empty($cekNoPromo->no_promo)) {
			...
			$this->model_penjualan->updateBuy1Get3Pending($cekNoPromo->no_promo, $noCart, $dataBut1Get3);
			$cekBuy1Get3Qty = $this->model_penjualan->cekBuy1Get3QtyPending($cekNoPromo->no_promo, $noCart, $sku);

			$gdiskon = (floor(($qty + $cekBuy1Get3Qty) / $cekNoPromo->qty_diskon)) * $cekNoPromo->discount;

			$cekBuy1Get3Exists = $this->model_penjualan->cekBuy1Get3ExistsPending($cekNoPromo->no_promo, $noCart);
			if (empty($cekBuy1Get3Exists)) {
				$dataBut1Get3 = array(
					"noCart" => $noCart,
					"no_promo" => $cekNoPromo->no_promo,
				...
				$this->model_penjualan->insertBuy1Get3Pending($dataBut1Get3);
			} else {
				$dataBut1Get3 = array(
					"diskon" => $gdiskon
				);
	...
	function diskonTebusMurah($sku, $qty)
	{
		...
		$cekEligibility = $this->db->get_where("ap_produk_tebusmurah_rules", array(
			"id_produk" => $sku,
			"id_toko" => $this->global['idStore'],
			"'" . date('Y-m-d') . "' between date_start and date_end"
		));
			...
			foreach ($cekEligibility->result() as $row) {
				...
				if ($row->nominal_belanja <= $cekSales && strtotime(date('Y-m-d')) >= strtotime($row->date_start) && strtotime(date('Y-m-d')) <= strtotime($row->date_end)) {
					if ($row->type == 1) {
						...
						$this->db->where("no_promo", $row->no_promo);
						$query = $this->db->get('ap_produk_tebusmurah_rules');
						$id_produk_list = [];

						foreach ($query->result() as $m) {
							$id_produk_list[] = $m->id_produk;
						}
						$terjual = $this->model_penjualan->cekQuotaDiskonGroup($id_produk_list, $row->date_start, $row->date_end, $row->JamMulai, $row->JamSelesai, $row->setJam, $row->discount, $this->global['idStore']);
					} else {
						$terjual = $this->model_penjualan->cekQuotaDiskonTebusMurah($sku, $row->date_start, $row->date_end, $row->JamMulai, $row->JamSelesai, $row->setJam, $row->discount, $this->global['idStore']);
					}
	...
	function updateQtyCart()
	{
		...
		if (!empty($cekNoPromo->no_promo)) {
			...
			$this->model_penjualan->updateBuy1Get3($cekNoPromo->no_promo, $idUser, $dataBut1Get3);
			$cekBuy1Get3Qty = $this->model_penjualan->cekBuy1Get3Qty($cekNoPromo->no_promo, $idUser, $sku);
	...
	function diskonPeritemPanel()
	{
		...
		if ($diskonPeritemPanel > 0) {
			$msg .= "<td colspan=2><i class='fa fa-bullhorn'></i> Diskon Peritem ";
			$msg .= "<b>" . number_format($diskonPeritemPanel, '0', ',', '.') . "</b>";
		}
		$diskonBuy1Get3 = $this->model_penjualan->diskonBuy1Get3($idUser);

		if ($diskonBuy1Get3 > 0) {

			$msg .= "<br><i class='fa fa-bullhorn'></i> Diskon <i>Get Free Item</i> ";
			$msg .= "<b>" . number_format($diskonBuy1Get3, '0', ',', '.') . "</b></td>";
		} else
			$msg .= "</td>";
		echo $msg;
	}
	...
	function diskonPeritemPanelPending()
	{
		...
		$diskonBuy1Get3 = $this->model_penjualan->diskonBuy1Get3Pending($idPending);

		if ($diskonBuy1Get3 > 0) {
			$msg .= "<td><i class='fa fa-bullhorn'></i> Diskon <i>Get Free Item</i></td>";
			$msg .= "<td align='right'>" . number_format($diskonBuy1Get3, '0', ',', '.') . "</td>";
		}
		echo $msg;
	}
	...
	function viewSurcharge()
	{
		...
		$poinReimburs = $this->model_penjualan->poinReimburs($idUser);
		$diskonVoucher = $this->model_penjualan->diskonVoucher($idUser);


		$diskonVoucherFisik = $this->model_penjualan->diskonVoucherFisik($idUser);

		$grandTotal = ($subtotal + $ongkir) - ($diskonPeritem + $diskonMember + $diskonPromosi + $poinReimburs + $diskonBuy1Get3 + $diskonVoucher + $diskonVoucherFisik);
		...
		if ($type_bayar != '2' && $type_bayar != '3') {
			$surcharge = 0;
			$this->model_penjualan->hapusSurcharge($idUser);
		} else {

			$subAccount = isset($_POST['subAccount']) ? $_POST['subAccount'] : 0;
			$surchargeSet = $this->model_penjualan->viewSurchargeSet($subAccount, $type_bayar);
	...
	function viewSurchargePending()
	{
		...
		$diskonBuy1Get3 = $this->model_penjualan->diskonBuy1Get3Pending($noCart);
	...
	function viewVoucher()
	{
		...
		// CART
		$this->db->select(array("ap_cart.quantity", "ap_cart.id_produk", "ap_cart.harga"));
		$this->db->from("ap_cart");
		$this->db->where("ap_cart.id_user", $idUser);
		$this->db->order_by("ap_cart.harga", "DESC");
		$cart = $this->db->get()->result_array();

		$bundles = $this->model_penjualan->cekBundling($this->global['idStore']);
		if ($bundles) {
			...
			foreach ($bundles as $bundle) {
				$bundle_products = json_decode($bundle->bundling_products, true);

				$discount_percent = $bundle->diskon / 100;

				// Hitung jumlah produk per bundling
				$bundle_counts = [];
				foreach ($bundle_products as $pid) {
					$bundle_counts[$pid] = 0;
				}

				foreach ($cart as $item) {
					if (in_array($item['id_produk'], $bundle_products)) {
						$bundle_counts[$item['id_produk']] += $item['quantity'];
					}
				}

				$bundling_qty = min($bundle_counts); // berapa kali bundling bisa berlaku
				//var_dump($bundle_counts);

				if ($bundling_qty > 0) {
					$bundle_total_price = 0;
					foreach ($cart as $item) {
						if (in_array($item['id_produk'], $bundle_products)) {
							$qty_for_bundle = min($bundling_qty, $item['quantity']);
							$bundle_total_price += $item['harga'] * $qty_for_bundle;
						}
					}
					$total_discount += $discount_percent * $bundle_total_price;
				}
			}
			...
			if ($total_discount > 0) {
				...
				$data = array(
					"idUser" => $idUser,
					"no_promo" => $bundle->no_promo,
					"diskon" => $total_discount
				);
				$this->model_penjualan->insertDiskonVoucher($data);

				$msg = "<td><i class='fa fa-credit-card'></i> Bundling Promo</td>";
				$msg .= "<td align='right'>" . number_format($total_discount, '0', ',', '.') . "</td>";
			...
			if ($cekPromo) {
				foreach ($cekPromo as $cek) {
					...
					if ($allGroupsMatched && $current_promo_total_qty >= $cek->jumlah_bayar) {
						...
						// Jika ada discount_percent atau discount_rp, pakai logika lama
						if (($cek->discount_percent > 0) || ($cek->discount_rp > 0)) {
							$potensi_diskon = ($current_promo_total_harga * ($cek->discount_percent / 100)) + $cek->discount_rp;
						}
			...
			$diskon_legacy = floatval($diskonBuy1Get3 ?: 0);

			if ($max_discount > 0 && $best_promo != null && $max_discount > $diskon_fisik && $max_discount > $diskon_legacy) {
				$this->model_penjualan->hapusCartVoucherFisik($idUser);
				$this->model_penjualan->hapusCartBuy1Get3($idUser);
				$this->model_penjualan->hapusCartVoucher($idUser);
				$data = array(
					"idUser"   => $idUser,
			...
			$cekVoucher = $this->model_penjualan->cekVoucher($id_customer);
			//var_dump($cekVoucher);

			$diskonVoucher = 0;
			foreach ($cekVoucher as $row) {
				$simulasi = $grandTotal - $diskonVoucher - $row->nominal;
				if ($simulasi >= 0) {

					$diskonVoucher += $row->nominal;
					$data = array(
						"idUser" => $idUser,
						"no_promo" => $row->id_voucer_member,
						"diskon" => $row->nominal
					);
					$this->model_penjualan->insertDiskonVoucher($data);
				}
			...
			if ($diskonVoucher > 0) {
				$msg = "<td><i class='fa fa-credit-card'></i> Voucher</td>";
				$msg .= "<td align='right'>" . number_format($diskonVoucher, '0', ',', '.') . "</td>";
	...
	function viewGrandTotal()
	{
		...
		$grandTotal = ($subtotal + $ongkir) - ($diskonPeritem + $diskonMember + $diskonPromosi + $poinReimburs + $diskonBuy1Get3 + $diskonVoucher + $diskonVoucherFisik) + $surcharge;
	...
	function viewGrandTotalPending()
	{
		...
		$grandTotal = ($subtotal + $ongkir) - ($diskonPeritem + $diskonMember + $diskonPromosi + $poinReimburs + $diskonBuy1Get3) + $surcharge;
	...
	function penjualan_sql()
	{
		...
		$seri_voucher = $this->input->post("seri_voucher");
		$seri_voucher = empty($seri_voucher) ? $this->model_penjualan->viewVoucherNoPromo($idUser) : $seri_voucher;
		$no_online = $this->input->post("no_online");

		if ($type_bayar == 3 || $type_bayar == 2) {
			$surcharge = $this->model_penjualan->viewSurcharge($idUser);
			if (empty($surcharge))
				$surcharge = 0;
		} else
		...
		$diskonBuy1Get3 = $this->model_penjualan->diskonBuy1Get3($idUser);
		$diskonVoucher = $this->model_penjualan->diskonVoucher($idUser);
		$diskonVoucher = empty($diskonVoucher) ? 0 : $diskonVoucher;
		$diskonVoucherFisik = $this->model_penjualan->diskonVoucherFisik($idUser);
		$diskonVoucher += empty($diskonVoucherFisik) ? 0 : $diskonVoucherFisik;
		$diskon_promosi = $diskon_promosi + $diskonBuy1Get3 + 0;
		...
		// Aktivasi poin untuk grosir kotty
		//update poin customer setelah transaksi 
		// 210124 
		//if($kategori_member!=2){
		if ($ulang_tahun == date('m-d') && $kategori_member != 2 && $kategori_member != 4 && $kategori_member > 0) {
			$poin = $poin * 2;
			$data_poin = array(
		...
		// Bila ada voucher, update status voucher
		if (!empty($seri_voucher)) {
			$data_voucher = array(
				"terpakai" => 1
			);

			$this->model_penjualan->updateVoucherSeri($seri_voucher, $data_voucher);
		}

		$diskon = empty($diskon) ? 0 : $diskon;
		$poin_lama = empty($poin_lama) ? 0 : $poin_lama;
		...
		$data_penjualan = array(
			...
			"poin" => $poin,
			"poin_before" => $poin_lama,
			"poin_reimburs" => $point_reimburs,
			"diskon_otomatis" => $diskon_otomatis,
			"seri_voucher" => $seri_voucher,
		...
		//var_dump($data_penjualan);echo $affect;

		//cek tipe bayar, jika 1 = piutang dan sisipkan ke tabel piutag
		if ($type_bayar == 5) {
			$tgl_jatuh_tempo = empty($this->input->post("jatuh_tempo")) ? strtotime('Y-m-d', "+30 days") : $this->input->post("jatuh_tempo");
			$data_piutang = array(
				"no_invoice" => $no_inv,
		...
		foreach ($viewCart as $row) {
			...
			if (!empty($row->no_promo)) {
				$this->db->set('quota_terpakai', 'quota_terpakai+' . $qty, FALSE);
				$this->db->where('id_produk', $sku);
		...
		// view cart
		$dataVoucher = $this->model_penjualan->dataVoucher($idUser);

		foreach ($dataVoucher as $ro) {
			$no_promo = $ro->no_promo;
			...
			$this->model_penjualan->updateVoucherMember($no_promo, $id_customer, $voucher_update);
		}
		...
		if ($this->global['idStore'] == 6 || $this->global['idStore'] == 5 || $this->global['idUser'] == 57) {
			...
			$this->db->where("no_online", $no_online);
			$this->db->where("status", 0);
			$this->db->update("ap_invoice_online", $update_resi);
		}
	...
	function cancelTrx()
	{
		...
		$this->db->insert_batch("ap_cart_deleted", $data_item);
	...
	function createQRDinamisBRI()
	{
		...
		//private key prepare
		$fp = fopen("keypair.pem", "r");
		$privKey = fread($fp, 8192);
		fclose($fp);

		$pKeyId = openssl_get_privatekey($privKey, 'optional_passphrase');

		$clientId = "eKKT0iySp5w3UbivxCuyOVvNiQcy1pzu";
		$clientSecret = "vkZAbONJDNd1Mbkx";
		$signatureToken = $this->generateSignatureToken($pKeyId, $clientId, $formattedTimestamp);
		...
		*/
		...
		$dataRequest = array(
			// 'partnerReferenceNo'=> "888801006159569",
			// 'amount'=>array('value'=>"61000.00",'currency'=>"IDR"),
			// 'merchantId'=>"000001000001178",
			// 'terminalId'=>"10049693"
			'partnerReferenceNo' => $_POST['partnerReferenceNo'],
			'amount' => array('value' => $_POST['amount'], 'currency' => "USD"),
			'merchantId' => $_POST['merchantId'],
			'terminalId' => $_POST['terminalId'],
			'additionalInfo' => array('referenceNo' => '220422001')
		);

		$bodyRequest = json_encode($dataRequest, true);
		//echo prettyPrint($bodyRequest);
		...
		$panjangRandom = 15;
		$randomNumber = $this->generateRandomNumber($panjangRandom);
		//Signature request
		$path = "/v1.0/qr-dynamic-mpm/qr-mpm-generate-qr";
		$method = "POST";
		$token = $accesstoken;
		$signatureRequest = $this->generateSignatureRequest($clientSecret, $method, $formattedTimestamp, $accesstoken, $bodyRequest, $path);
		...
		//CURL Request Fitur
		$urlPostrequest = "https://sandbox.partner.api.bri.co.id/v1.0/qr-dynamic-mpm/qr-mpm-generate-qr";
		$chPost1 = curl_init();
		...
		// $_SESSION['resultPostRequest']=$resultPostRequest;

		$hasil = json_decode($resultPostRequest, true);
		//echo "<h4>".$hasil['qrContent']."</h4>";
		echo "<img src='barcode.php?h=500&f=png&s=qr&d=" . $hasil['qrContent'] . "'>";
	}
}
