...
class Laporan extends BaseController{
	...
	function openKey(){
		$pin 		= $_POST['pin'];
		$this->db->select("openkey");
		$this->db->from("user_openkey");
		$this->db->where("openkey",$pin);
		$query = $this->db->get()->result();
		$key = md5('124f'.rand(209,5000));
	...
	function topBrand(){
		if (!$this->ion_auth->logged_in()){	
			...
			$cekMyAccess = $this->model1->cekMyAccess($this->global['idUser'],6,79);
			if($cekMyAccess < 1){
				$this->accessDenied();
			} else {
				...
				$data['pageTitle'] = "SOLUSI POS - Top Brand";
				...
				$this->load->view("laporan_penjualan/bodyTopBrand",$data);
				$this->load->view("laporan_penjualan/jsInclude");
				$this->load->view("laporan_penjualan/js/jsTopBrand");
				$this->load->view("laporan_penjualan/closeTag");
			}
		}
	}
	function laporanTopBrand(){
		...
		$data['laporan'] = $this->modelLaporan->topBrand($dateStart, $dateEnd,$toko,$kategori,$subkategori,$subkategori2);
		$this->load->view("laporan_penjualan/laporanTopBrand",$data);
	}
	function exportExcelLaporanPenjualanPerkasirDetail(){
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1','No')
									  ...
									  ->setCellValue('E1','No. Invoice')
									  ->setCellValue('F1','Harga Jual')
									  ->setCellValue('G1','Qty')
									  ->setCellValue('H1','Total Harga')
									  ->setCellValue('I1','Total Diskon')
									  ->setCellValue('J1','Diskon Supplier')
		...
		$laporan = $this->modelLaporan->penjualanPerkasirDetail($start, $end,$idKasir,$idStore,$idPayment,$isAdmin);
		$i=2;
		foreach($laporan->result() as $row){
			...
			$objPHPExcel->getActiveSheet()->setCellValue('A'.$i,$i-1)
									  ...
									  ->setCellValue('E'.$i,$row->no_invoice)
									  ->setCellValue('F'.$i,$row->harga_jual)
									  ->setCellValue('G'.$i,$row->qty)
									  ->setCellValue('H'.$i,$row->harga_jual*$row->qty)
									  ->setCellValue('I'.$i,$row->diskon)
									  ->setCellValue('J'.$i,$row->disc_supplier)
									  ->setCellValue('K'.$i,($row->harga_jual*$row->qty)-$row->diskon)
									  ->setCellValue('L'.$i,$row->brand);
	    ...
	    header('Content-Disposition: attachment;filename=Laporan Penjualan Per Kasir Detail.xlsx');
	    //unduh file
	    $objWriter->save("php://output");

	}
	function exportExcelLaporanPenjualanPerkasirDetail2(){
		$file="export struk.xls";
		...
		$laporan = $this->modelLaporan->penjualanPerkasirDetail($start, $end,$idKasir,$idStore,$idPayment,$isAdmin);
		$i=2;$html = '<table border=0 width=900>';$no_invoice = '';$item = 0;$belanja = 0;$diskon = 0; $total =0; 
		foreach($laporan->result() as $row){
			if($no_invoice!=$row->no_invoice){
				if($item>0){
					$html .="<tr><td colspan=2 align=center><b>Total Item</td><td><b>".$item."</td></tr>";
					$html .="<tr><td colspan=2 align=center><b>Total Belanja</td><td><b>".number_format($belanja,0,',','.')."</td></tr>";
					$html .="<tr><td colspan=2 align=center><b>Total Diskon Item</td><td><b>".number_format($diskon,0,',','.')."</td></tr>";
					$html .="<tr><td colspan=2 align=center><b>Grand Total</td><td><b>".number_format($total,0,',','.')."</td></tr></table><table border=0>";
				...
				$html .="<tr><td width=40%>No. Invoice</td><td>:</td><td>".$row->no_invoice."</td></tr>";
				$html .="<tr><td>Tanggal</td><td>:</td><td>".$row->tanggal."</td></tr>";
				$html .="<tr><td>Keterangan</td><td>:</td><td>".$row->keterangan."</td></tr><table><table border=1 cellpadding=4 rowspacing=4>";
				$html .="<tr><td><b>Produk</td><td><b>Diskon</td><td><b>Total</td></tr>";
				
			}
			$item++;
			$belanja += $row->harga_jual*$row->qty;
			$diskon += $row->diskon;
			$total += ($row->harga_jual*$row->qty) - $row->diskon;

			$html .="<tr><td>".$row->nama_produk."</td><td rowspan=2>".number_format($row->diskon,0,',','.')."</td><td rowspan=2>".number_format(($row->harga_jual*$row->qty)-$row->diskon,0,',','.')."</td></tr>";
			$html .="<tr><td>".$row->qty." x ".number_format($row->harga_jual,0,',','.')."</td></tr>";
		...
		$html .="<tr><td colspan=2 align=center><b>Total Belanja</td><td><b>".number_format($belanja,0,',','.')."</td></tr>";
		$html .="<tr><td colspan=2 align=center><b>Total Diskon Item</td><td><b>".number_format($diskon,0,',','.')."</td></tr>";
		$html .="<tr><td colspan=2 align=center><b>Grand Total</td><td><b>".number_format($total,0,',','.')."</td></tr></table>";
		echo $html;
	...
	function penjualan_brand(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$this->global['pageTitle'] = "SOLUSI POS - Laporan Penjualan";
				$this->global['navigation'] = $this->model1->callNavigation();
				$this->loadViews("laporan/body_laporan_penjualan_brand",$this->global,NULL,"footer_empty");
			}
		}
	}
	...
	function pembelian(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$this->global['pageTitle'] = "SOLUSI POS - Laporan Pembelian";
	...
	function waste(){
		$this->global['pageTitle'] = "SOLUSI POS - Laporan Waste";
	...
	function stock_opname(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$this->global['pageTitle'] = "SOLUSI POS - Laporan Stock Opname";
				$this->global['navigation'] = $this->model1->callNavigation();
	...
	function hutang_po(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$this->global['pageTitle'] = "SOLUSI POS - Laporan Hutang Pembelian";
	...
	function hutang_jatuh_tempo(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$this->global['pageTitle'] = "SOLUSI POS - Laporan Hutang Jatuh Tempo";
				$data['header'] = $this->db->get("ap_receipt")->row();
				$data['supplier'] = $this->db->get("supplier")->result();
	...
	function hutang_terbayar(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$this->global['pageTitle'] = "SOLUSI POS - Laporan Hutang Terbayar";
				$data['supplier'] = $this->db->get("supplier")->result();
	...
	function analisa_umur_hutang(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$this->global['pageTitle'] = "SOLUSI POS - Analisa Umur Hutang";
				$data['supplier'] = $this->db->get("supplier")->result();
	...
	function penjualan_perbarang(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$data['pageTitle'] = "SOLUSI POS - Penjualan Perbarang";
				$data['navigation'] = $this->model1->callNavigation();
				$data['permitAccess'] = $this->model1->permitAccess($this->global['idUser']);
				$data['permitAccessSub'] = $this->model1->permitAccessSub($this->global['idUser']);
				$this->load->view("navigation",$data);
	...
	function akumulasiPenjualanProdukPerkriteria(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$data['permitAccess'] = $this->model1->permitAccess($this->global['idUser']);
				$data['permitAccessSub'] = $this->model1->permitAccessSub($this->global['idUser']);
				$data['pageTitle'] = "SOLUSI POS - Akumulasi Penjualan Berdasarkan Produk";
				$this->load->view("navigation",$data);
	...
	function akumulasiPenjualanPerkriteria(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$data['permitAccess'] = $this->model1->permitAccess($this->global['idUser']);
				$data['permitAccessSub'] = $this->model1->permitAccessSub($this->global['idUser']);
				$data['pageTitle'] = "SOLUSI POS - Akumulasi Penjualan Berdasarkan Invoice";
	...
	function laporanPenjualanPerkriteriaProduk(){
		...
		$data['timeStart'] 		= $_POST['timeStart'].':00';
		$data['timeEnd'] 		= $_POST['timeEnd'].':00';

		$data['laporan'] = $this->modelLaporan->penjualanPerkriteriaProduk($dateStart, $dateEnd,$toko,$tempat,$customer,$kategori,$subkategori,$subkategori2,$id_supplier,$id_brand,$timeStart,$timeEnd);
	...
	function exportPDFLaporanPenjualanPerkriteriaProduk(){
		...
		$timeStart 		= $_GET['timeStart'].':00';
		$timeEnd 		= $_GET['timeEnd'].':00';
		...
		$string = $toko==7? "Stok Gudang":'';
		$string2 = $toko==7? "Terjual Gudang":'';

		$pdf->AddPage();
		// setting jenis font yang akan digunakan
        $pdf->SetFont('Arial','B',16);
        // mencetak string 
        $pdf->Cell(270,7,'Akumulasi Penjualan Per Produk',0,1,'C');
        $pdf->SetFont('Arial','B',12);
        $pdf->Cell(270,7,date('d F Y',strtotime($dateStart)).' - '.date('d F Y',strtotime($dateEnd)),0,1,'C');
		...
		$pdf->Cell(100,6,'Nama Produk',1,0,'L');
		$pdf->Cell(15,6,'Terjual',1,0,'C');
		$pdf->Cell(25,6,'Nilai Terjual',1,0,'C');
		$pdf->Cell(25,6,'Setelah Diskon',1,0,'C');
		$pdf->Cell(15,6,'Stok',1,0,'C');
		$pdf->Cell(25,6,$string,1,0,'C');
		$pdf->Cell(25,6,$string2,1,0,'C');
		$pdf->Ln();
		$pdf->SetFont('Arial','',9);

		$laporan = $this->modelLaporan->penjualanPerkriteriaProduk($dateStart, $dateEnd,$toko,$tempat,$customer,$kategori,$subkategori,$subkategori2,$id_supplier,$id_brand,$timeStart,$timeEnd);

		$i = 1;
		foreach($laporan->result() as $row){
			if($toko==7){
				$H1 = $this->modelLaporan->stokToko($row->id_produk,6);
				$T1	= $this->modelLaporan->terjualToko($row->id_produk,6,$dateStart, $dateEnd);
			}else{
				$H1 = '';
				$T1 = '';
			}
			...
			$pdf->Cell(30,6,$row->id_produk,1,0,'L');
			$pdf->Cell(100,6,$row->nama_produk,1,0,'L');
			$pdf->Cell(15,6,$row->qty_terjual,1,0,'C');
			$pdf->Cell(25,6,number_format($row->harga_jual*$row->qty_terjual,0,',','.'),1,0,'R');
			$pdf->Cell(25,6,number_format(($row->harga_jual*$row->qty_terjual)-$row->diskon,0,',','.'),1,0,'R');
			$pdf->Cell(15,6,$row->stok,1,0,'C');
			$pdf->Cell(25,6,$H1,1,0,'C');
			$pdf->Cell(25,6,$T1,1,0,'C');
			$pdf->Ln();
		...
		$pdf->SetFont('Arial','B',9);
		$pdf->Cell(140,6,"T O T A L",1,0,'C');
		$pdf->Cell(15,6,$qty_terjual,1,0,'C');
		$pdf->Cell(25,6,number_format($penjualan,0,',','.'),1,0,'R');
		$pdf->Cell(25,6,number_format($grand_total,0,',','.'),1,0,'R');
		$pdf->Cell(15,6,$stok,1,0,'C');
		$pdf->Cell(25,6,$H1total,1,0,'C');
		$pdf->Cell(25,6,$T1total,1,0,'C');
		$pdf->Ln();
	...
	function exportExcelLaporanPenjualanPerkriteriaProduk(){
		...
		$string2 = $toko==7? "Terjual Gudang":'';
			...
			$objPHPExcel->getActiveSheet()->setCellValue('A1','No')
									  ...
									  ->setCellValue('N1','Brand')
										...
										->setCellValue('V1',$string)
										->setCellValue('W1',$string2)
										->setCellValue('X1','Nomor Resi');
		...
		foreach($laporan->result() as $row){
			if($toko==7){
				...
				$H1 = $this->modelLaporan->stokToko($row->id_produk,5);
				$T1	= $this->modelLaporan->terjualToko($row->id_produk,5,$dateStart, $dateEnd);
			}else{
				$H1 = '';
				$T1 = '';
			}
			...
			$objPHPExcel->getActiveSheet()->setCellValue('A'.$i,$i-1)
                                     ...
                                     ->setCellValue('N'.$i,$row->brand)   
										...
										->setCellValue('V'.$i,$H1)
										->setCellValue('W'.$i,$T1)
										->setCellValueExplicit('X'.$i,$row->no_resi, PHPExcel_Cell_DataType::TYPE_STRING);
				$qty_terjual +=$row->qty_terjual;
				$modal += $row->harga_beli*$row->qty_terjual;
				$penjualan += $row->harga_jual*$row->qty_terjual;
				$diskon += $row->diskon;
		...
		$penjualanPerkriteriaProdukTidakTerjual = $this->modelLaporan->penjualanPerkriteriaProdukTidakTerjual($dateStart, $dateEnd,$toko,$tempat,$customer,$kategori,$subkategori,$subkategori2,$id_supplier,$id_brand);


		foreach($penjualanPerkriteriaProdukTidakTerjual->result() as $row){
			if($toko==7){
				$H1 = $this->modelLaporan->stokToko($row->id_produk,6);
				$T1	= $this->modelLaporan->terjualToko($row->id_produk,6,$dateStart, $dateEnd);
			}else{
			...
			$est =number_format((0.2/$day)*$dayMonth,2);
			$aging = number_format($row->stok/(0.6/$day),0);
			$ssr = number_format($row->stok/$est,2);
			$objPHPExcel->getActiveSheet()->setCellValue('A'.$i,$i-1)
									  ->setCellValueExplicit('B'.$i,$row->id_produk, PHPExcel_Cell_DataType::TYPE_STRING)
									  ->setCellValue('C'.$i,$row->nama_produk)
	...
	function exportExcelLaporanPenjualanPerkriteriaProdukPisahINV(){
		//error_reporting(0); ini_set('display_errors',0);
		$this->load->library("excel/PHPExcel");
		...
		$string2 = $toko==7? "Terjual Gudang":'';
			...
			$objPHPExcel->getActiveSheet()->setCellValue('A1','No')
									  ...
									  ->setCellValue('M1','Profit')
									  ->setCellValue('N1','Brand')
										->setCellValue('O1','No. Invoice')
										->setCellValue('P1','Nomor Resi');
		...
		$laporan = $this->modelLaporan->penjualanPerkriteriaProdukPisahINV($dateStart, $dateEnd,$toko,$tempat,$customer,$kategori,$subkategori,$subkategori2,$id_supplier,$id_brand);
		...
		foreach($laporan->result() as $row){
			$objPHPExcel->getActiveSheet()->setCellValue('A'.$i,$i-1)
									  ...
									  ->setCellValue('M'.$i,($row->harga_jual*$row->qty_terjual)-$row->diskon-($row->harga_beli*$row->qty_terjual))
                                     ->setCellValue('N'.$i,$row->brand) 
									 ->setCellValue('O'.$i,$row->no_invoice)  
										->setCellValue('P'.$i,$row->no_resi);
				$qty_terjual +=$row->qty_terjual;
				$modal += $row->harga_beli*$row->qty_terjual;
				$penjualan += $row->harga_jual*$row->qty_terjual;
				$diskon += $row->diskon;
	    ...
	    header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
	    //ubah nama file saat diunduh
	    header('Content-Disposition: attachment;filename=Laporan Penjualan Pisah INV.xlsx');
	    //unduh file
	    $objWriter->save("php://output");
	} 
	...
	function exportExcelLaporanPenjualanPerkriteria(){
		...
		foreach($laporan->result() as $row){
			$objPHPExcel->getActiveSheet()->setCellValue('A'.$i,$i-1)
									  ...
									  ->setCellValueExplicit('M'.$i,$row->nama_penerima, PHPExcel_Cell_DataType::TYPE_STRING);
		$i++; }
	...
	function penjualan_perkategori_customer(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$data['permitAccess'] = $this->model1->permitAccess($this->global['idUser']);
				$data['permitAccessSub'] = $this->model1->permitAccessSub($this->global['idUser']);
				$data['pageTitle'] = "SOLUSI POS - Penjualan Perkategori Customer";
	...
	function penjualan_percustomer(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$data['pageTitle'] = "SOLUSI POS - Penjualan Percustomer";
				$data['navigation'] = $this->model1->callNavigation();
				$data['permitAccess'] = $this->model1->permitAccess($this->global['idUser']);
				$data['permitAccessSub'] = $this->model1->permitAccessSub($this->global['idUser']);
				$this->load->view("navigation",$data);
	...
	function penjualan_perkategori_produk(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$data['permitAccess'] = $this->model1->permitAccess($this->global['idUser']);
				$data['permitAccessSub'] = $this->model1->permitAccessSub($this->global['idUser']);
				$data['pageTitle'] = "SOLUSI POS - Penjualan Perkategori Produk";
				$data['navigation'] = $this->model1->callNavigation();
				$this->load->view("navigation",$data);
	...
	function penjualan_pertoko(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$data['permitAccess'] = $this->model1->permitAccess($this->global['idUser']);
				$data['permitAccessSub'] = $this->model1->permitAccessSub($this->global['idUser']);
				$data['pageTitle'] = "SOLUSI POS - Penjualan Pertoko";
	...
	function penjualan_perkasir(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$data['permitAccess'] = $this->model1->permitAccess($this->global['idUser']);
				$data['permitAccessSub'] = $this->model1->permitAccessSub($this->global['idUser']);
				$data['pageTitle'] = "SOLUSI POS - Penjualan Perkasir";
		
				$this->load->view("navigation",$data);
				$data['listKasir'] 	= $this->modelLaporan->list_kasir();
				$data['listPayment']= $this->modelLaporan->list_payment();
	...
	function penjualanPerkasirReport(){
		...
		$data['end'] 		= $_POST['dateEnd'];
		$data['idKasir'] 	= $_POST['idKasir'];
		$data['idPayment']	= $_POST['idPayment'];
		...
		$data['laporanPerkasir'] = $this->modelLaporan->penjualanPerkasir($start,$end,$idKasir,$this->global['idStore'],$this->global['idUser'],$isAdmin,$idPayment);
	...
	function akumulasi_penjualan_produk(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$data['permitAccess'] = $this->model1->permitAccess($this->global['idUser']);
				$data['permitAccessSub'] = $this->model1->permitAccessSub($this->global['idUser']);
				$data['pageTitle'] = "SOLUSI POS - Akumulasi Penjualan Produk";
	...
	function akumulasiPenjualanProdukReport(){
		...
		$data['laporanAkumulasiProduk'] = $this->modelLaporan->akumulasiPenjualanProduk($start,$end,$id_toko,$this->global['idUser'],$this->global['isAdmin']);
	...
	function exportExcelakumulasiPenjualanProduk(){
		...
		$laporan = $this->modelLaporan->akumulasiPenjualanProduk($start,$end,$this->global['idStore'],$this->global['idUser'],$this->global['isAdmin']);
	...
	function akumulasi_penjualan_supplier(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$data['permitAccess'] = $this->model1->permitAccess($this->global['idUser']);
				$data['permitAccessSub'] = $this->model1->permitAccessSub($this->global['idUser']);
				$data['pageTitle'] = "SOLUSI POS - Akumulasi Penjualan Brand";
				if($this->global['isSuperadmin']==1){
					$data['toko'] = $this->db->get("ap_store")->result();
				}else $data['toko'] = $this->db->get_where("ap_store",array('id_store' => $this->global['idStore']))->result();
	...
	function akumulasi_pembelian_brand(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$data['permitAccess'] = $this->model1->permitAccess($this->global['idUser']);
				$data['permitAccessSub'] = $this->model1->permitAccessSub($this->global['idUser']);
				$data['pageTitle'] = "SOLUSI POS - Akumulasi Pembelian Brand";
				if($this->global['isSuperadmin']==1){
					$data['toko'] = $this->db->get("ap_store")->result();
				}else $data['toko'] = $this->db->get_where("ap_store",array('id_store' => $this->global['idStore']))->result();
	...
	private function _top_customer_filter_ids($key){
	...
	private function _top_customer_totals_mode(){
		return ($this->input->get('totals_mode') === 'peritem') ? 'peritem' : 'global';
	}
	...
	function top_customer(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$data['permitAccess'] = $this->model1->permitAccess($this->global['idUser']);
				$data['permitAccessSub'] = $this->model1->permitAccessSub($this->global['idUser']);
				$data['pageTitle'] = "SOLUSI POS - Top Customer";
		
				$this->load->view("navigation",$data);

				$start = isset($_GET['dateStart'])?$_GET['dateStart']:'';
				$end = isset($_GET['dateEnd'])?$_GET['dateEnd']:'';
				$idToko = isset($_GET['id_toko'])?$_GET['id_toko']:'';
				$idBrand = $this->_top_customer_filter_ids('id_brand');
				$idGroup = $this->_top_customer_filter_ids('id_group');
				$totalsMode = $this->_top_customer_totals_mode();

				$total_rows = $this->model1->top_customer_count($start,$end,$idToko,$idBrand,$idGroup,$totalsMode);
				$this->load->library('pagination');
				...
				$data['date_end'] = $end;
				$data['id_brand_sel'] = $idBrand;
				$data['id_group_sel'] = $idGroup;
				$data['totals_mode'] = $totalsMode;
				$data['brands'] = $this->db->order_by('brand','ASC')->get('brand')->result();
				$data['customer_groups'] = $this->db->order_by('group_customer','ASC')->get('ap_customer_group')->result();
				$data['idStore'] = $this->global['idStore'];
				$data['isAdmin'] = $this->global['isAdmin'];

				$data['top_customer'] = $this->model1->top_customer($per_page,$page,$start,$end,$idToko,$idBrand,$idGroup,$totalsMode);
	...
	function export_csv_top_customer(){
		if (!$this->ion_auth->logged_in()){
			redirect("login");
		}
		$cekMyAccess = $this->model1->cekMyAccess($this->global['idUser'],2,21);
		...
		$idToko = $this->input->get('id_toko');
		$idBrand = $this->_top_customer_filter_ids('id_brand');
		$idGroup = $this->_top_customer_filter_ids('id_group');
		$totalsMode = $this->_top_customer_totals_mode();
		$d = $this->input->get('delimiter');
		$delimiter = ($d === ';') ? ';' : ',';

		$rows = $this->model1->top_customer_all($start,$end,$idToko,$idBrand,$idGroup,$totalsMode);
		$fn = 'top_customer_'.preg_replace('/[^0-9\-]/','',$start).'_'.preg_replace('/[^0-9\-]/','',$end).'.csv';

		header('Content-Type: text/csv; charset=utf-8');
		header('Content-Disposition: attachment; filename="'.$fn.'"');
		$out = fopen('php://output','w');
		fwrite($out,"\xEF\xBB\xBF");
		$hdr = array('No','Nama Customer','ID Customer','Kontak','Tipe Customer','Subtotal','Diskon Channel','Diskon','Poin Reimburs','Diskon Peritem','Total','Kedatangan','Transaksi Terakhir');
		fputcsv($out,$hdr,$delimiter);
		$i = 1;
		foreach ($rows as $row){
			fputcsv($out,array(
				$i,
				...
				$row->id_customer,
				$row->kontak,
				isset($row->group_customer) ? $row->group_customer : '',
				$row->total,
				$row->diskon,
				$row->diskon_free,
				$row->poin_value,
	...
	function top_customer_presensce(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$data['permitAccess'] = $this->model1->permitAccess($this->global['idUser']);
				$data['permitAccessSub'] = $this->model1->permitAccessSub($this->global['idUser']);
				$data['pageTitle'] = "SOLUSI POS - Top Customer by Presence";
				...
				$data['top_customer'] = $this->model1->top_customer_presence($per_page,$page,$start,$end,$idToko,$idBrand,$idGroup,$totalsMode);
	...
	function export_csv_top_customer_presence(){
		if (!$this->ion_auth->logged_in()){
			redirect("login");
		}
		$cekMyAccess = $this->model1->cekMyAccess($this->global['idUser'],2,21);
		...
		$rows = $this->model1->top_customer_presence_all($start,$end,$idToko,$idBrand,$idGroup,$totalsMode);
		$fn = 'top_customer_presence_'.preg_replace('/[^0-9\-]/','',$start).'_'.preg_replace('/[^0-9\-]/','',$end).'.csv';
	...
	function mutasiBarang(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$this->global['pageTitle'] = "SOLUSI POS - Laporan Barang Keluar / Mutasi";
				$this->global['navigation'] = $this->model1->callNavigation();
	...
	function mutasiPeritem(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$this->global['pageTitle'] = "SOLUSI POS - Mutasi Peritem";
	...
	function transferStok(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$this->global['pageTitle'] = "SOLUSI POS - Laporan Transfer Stok";
				$this->global['navigation'] = $this->model1->callNavigation();
	...
	function transferStokPeritem(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$this->global['pageTitle'] = "SOLUSI POS - Transfer Stok Peritem";
				$this->global['navigation'] = $this->model1->callNavigation();
	...
	function invoice_penjualan(){
		$data['pageTitle'] = "SOLUSI POS - Invoice Penjualan";
	...
	function suratJalan(){
		$data['pageTitle'] = "SOLUSI POS - Surat Jalan";
		$noInvoice = $this->input->get('no_invoice');
		$idStore = $this->model_penjualan->getIdStore($noInvoice);
		$data['header'] = $this->db->get_where("ap_store",array("id_store" => $idStore))->row();
		$data['invoiceInfo'] = $this->model_penjualan->invoiceInfo($noInvoice);
	...
	function penerimaanBarang(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$this->global['pageTitle'] = "SOLUSI POS - Laporan Penerimaan Barang";
				$this->global['navigation'] = $this->model1->callNavigation();
	...
	function penerimaanBarangPeritem(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$data['supplier'] = $this->db->get("supplier")->result();
				$this->global['pageTitle'] = "SOLUSI POS - Penerimaan Barang Peritem";
				$this->global['navigation'] = $this->model1->callNavigation();
	...
	function returPembelian(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$this->global['pageTitle'] = "SOLUSI POS - Laporan Retur Pembelian";
				$data['supplier'] = $this->db->get("supplier")->result();
	...
	function returPenjualan(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$this->global['pageTitle'] = "SOLUSI POS - Laporan Retur Penjualan";
				if($this->global['isSuperadmin']==1){
					$data['toko'] = $this->db->get("ap_store")->result();
				}else $data['toko'] = $this->db->get_where("ap_store",array('id_store' => $this->global['idStore']))->result();
	...
	function setoranKasir(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$this->global['pageTitle'] = "SOLUSI POS - Laporan Setoran Kasir";
				if($this->global['isSuperadmin']==1){
					$data['toko'] = $this->db->get("ap_store")->result();
				}else $data['toko'] = $this->db->get_where("ap_store",array('id_store' => $this->global['idStore']))->result();
	...
	function labaRugi(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$this->global['pageTitle'] = "SOLUSI POS - Laporan Laba Rugi";
				if($this->global['isSuperadmin']==1){
					$data['toko'] = $this->db->get("ap_store")->result();
				}else $data['toko'] = $this->db->get_where("ap_store",array('id_store' => $this->global['idStore']))->result();
	...
	function produkPromo(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$this->global['pageTitle'] = "SOLUSI POS - Laporan Produk Promo";
				if($this->global['isSuperadmin']==1){
					$data['store'] = $this->db->get("ap_store")->result();
				}else $data['store'] = $this->db->get_where("ap_store",array('id_store' => $this->global['idStore']))->result();
				$data['tempat'] = $this->db->get("ap_stand")->result();
	...
	function penjualanPerkategori(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$this->global['pageTitle'] = "SOLUSI POS - Laporan Penjualan Per Departemen";
				if($this->global['isSuperadmin']==1){
					$data['store'] = $this->db->get("ap_store")->result();
				}else $data['store'] = $this->db->get_where("ap_store",array('id_store' => $this->global['idStore']))->result();
	...
	function penjualanDibatalkan(){
		if (!$this->ion_auth->logged_in()){	
			...
			if($cekMyAccess < 1){
				...
				$this->load->model("ModelSalesByKategori");
				$this->global['pageTitle'] = "SOLUSI POS - Laporan Transaksi dibatalkan";
				if($this->global['isSuperadmin']==1){
					$data['store'] = $this->db->get("ap_store")->result();
				}else $data['store'] = $this->db->get_where("ap_store",array('id_store' => $this->global['idStore']))->result();
