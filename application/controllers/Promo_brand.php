...
class Promo_brand extends BaseController{
	function __construct(){
		parent::__construct();
		$this->load->helper("url");
		$this->load->library(array('session','encryption'));
		$this->load->model(array("model1","modelPromoBrand"));
		$this->load->database();
	...
	function index(){
		$this->global['pageTitle'] = "SOLUSI POS - Promo Brand";
		$this->global['navigation'] = $this->model1->callNavigation();
		$data['store'] = $this->db->get("ap_store");
		$data['brand'] = $this->db->get("brand");
		$this->loadViews("promo_brand/body",$this->global,$data,"promo_brand/footer");
	}
	...
	function insertPO(){
		...
		$cek_tanggal 	= $this->modelPromoBrand->cek_tanggal_promo($tanggal_po);
		...
		$this->modelPromoBrand->insertPONumber($data_masuk);
	...
	function daftar_po(){
		$this->global['pageTitle'] = "SOLUSI POS - Informasi Promo Brand";
		$this->global['navigation'] = $this->model1->callNavigation();
		$this->loadViews("promo_brand/body_daftar",$this->global,NULL,"promo_brand/footerDaftarPO");
	}
	...
	function form_promo(){
		...
		$info_po = $this->modelPromoBrand->infoPurchase($no_po);
		...
		$data['rules_type'] 		= $info_po->rules_type;
		...
		$this->global['navigation'] = $this->model1->callNavigation();
		$this->loadViews("promo_brand/body_form_po",$this->global,$data,"promo_brand/footer_barang_masuk");
	}
	...
	function datatablesPO(){
		...
		$total 			 			= $this->modelPromoBrand->totalPromoSupplier();
		$output 					= array();
		$output['draw']	 			= $draw;
		...
		if($search!=""){
			$query = $this->modelPromoBrand->viewPOProduk($length,$start,$search,$idUser,$this->global['idStore']);
		...
		foreach ($query->result_array() as $dt) {
			...
			$output['data'][]=array($nomor_urut,"<a href='".base_url('promo_brand/form_promo?no_promo='.$dt['no_promo'])."'>".$dt['no_promo']."</a>",$dt['tanggal_buat'],$dt['date_start'],$dt['date_end'],$dt['brand'],$dt['rules_type'],$dt['minBelanja'],$dt['discount'],$dt['first_name'],"<a onclick=\"if (confirm('Yakin hapus semua data yang berhubungan dengan nomor promo ini?')){hapusPromo('".$dt['no_promo']."');}\" ><i class=\"fa fa-trash\"></i></a>");
			$nomor_urut++;
		}
