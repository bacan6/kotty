...
class Data_stok extends BaseController{
	...
	function index(){
		...
		$this->global['pageTitle'] = "Solusinformatika.com - Data Stok Gudang";
		$this->loadViews("data_stok/body_data_stok_fg",$this->global,$data,"data_stok/footerDataStok");
	}

	function stokPerkategori(){
		if (!$this->ion_auth->logged_in()){	
			redirect("login");
		} else {
			$this->load->model("ModelStokByKategori");
			$this->global['pageTitle'] = "Solusinformatika.com - Laporan Stok Per Departemen";
			$data['store'] = $this->db->get("ap_store")->result();
			$data['brand'] = $this->db->get("brand")->result();
			$this->loadViews("data_stok/stokPerkategori",$this->global,$data,"data_stok/footerStokPerkategori");
		}
	}

	function viewReportStokPerkategori(){
		$this->load->model("ModelStokByKategori");
		...
		$this->load->view("data_stok/viewStokByKategori",$data);
	}
	...
	function exportExcelFgFilter(){
		...
		$data_stok = $this->modelDataStok->dataStokFgFilterExport($idKategori,$subkategori,$subSubKategori,$stokSign,$stokValue,$priceSign,$priceSignValue);
	...
	function datatablesStokFg(){
		...
		foreach ($query->result_array() as $dt) {
			$output['data'][]=array($nomor_urut,$dt['id_produk'],$dt['nama_produk'],$dt['kategori']."-".$dt['kategori_level_1']."-".$dt['kategori_3'],number_format($dt['stok'],'0',',',''),number_format($dt['harga'],'0',',','.'));
			$nomor_urut++;
		}
	...
	function datatablesStokFgFilter(){
		...
		$total 			 			= $this->modelDataStok->totalProdukActivefilter($idKategori,$subkategori,$subSubKategori,$stokSign,$stokValue,$priceSign,$priceSignValue);
		$output 					= array();
		$output['draw']	 			= $draw;
		$output['recordsTotal'] 	= $output['recordsFiltered']=$total;
		$output['data'] 			= array();
		...
		if($search!=""){
			$query = $this->modelDataStok->dataStokFgFilter($length,$start,$search,$idKategori,$subkategori,$subSubKategori,$stokSign,$stokValue,$priceSign,$priceSignValue);
			$output['recordsTotal'] = $output['recordsFiltered'] = $query->num_rows();
		} else {
			$query = $this->modelDataStok->dataStokFgFilter($length,$start,$search,$idKategori,$subkategori,$subSubKategori,$stokSign,$stokValue,$priceSign,$priceSignValue);
		}
