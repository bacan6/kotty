...
Class ModelFinance extends CI_Model{
	...
	function total_tagihan($store,$supplier,$terbayar){
		$this->db->select("hutang.no_tagihan");
		$this->db->from("hutang");
		...
		if(!empty($terbayar)){
			$terbayar = ($terbayar == 'NO') ? '0' : $terbayar;
			$this->db->where("hutang.status_hutang",$terbayar);
		}
	...
	function dataTagihan($limit,$start,$search,$idUser='',$idStore='',$supplier,$terbayar){
		$this->db->select(array("hutang.no_tagihan","purchase_order.tanggal_po","purchase_order.jatuh_tempo","users.first_name","hutang.status_hutang","receive_order.keterangan_receive","receive_order.status_receive","purchase_order.keterangan","SUM(receive_item.price*receive_item.qty) as total","hutang.status_hutang","supplier.supplier"));
		$this->db->from("hutang");
		$this->db->join("purchase_order","purchase_order.no_po = hutang.no_tagihan","left");
		...
		if(!empty($search)){
			...
			$this->db->like("hutang.no_tagihan",$search);
			$this->db->or_like("supplier.supplier",$search);
			$this->db->or_like("purchase_order.keterangan",$search);
		}
		if(!empty($idStore)){
	...
	function purchaseItem($no_po){
		$this->db->select(array("ap_produk.nama_produk","receive_item.qty","receive_item.ppn","ap_produk.satuan","receive_item.price as harga","(receive_item.price*receive_item.qty) as total","ap_produk.id_produk","receive_order.diskon"));
		$this->db->from("purchase_item");
		$this->db->join("ap_produk","ap_produk.id_produk = purchase_item.sku","left");
		$this->db->join("receive_order","receive_order.no_po = purchase_item.no_po","left");
		$this->db->join("receive_item","receive_item.sku=purchase_item.sku and receive_item.no_receive = receive_order.no_receive","left");
	...
	function diskonProduk($no_po,$sku){
		$this->db->select("SUM(diskon1/100*(qty*price)) as diskon1,SUM(diskon2/100*((qty*price)-diskon1/100*(qty*price))) as diskon2,SUM(diskon3*qty) as diskon3");
		$this->db->from("receive_item");
		$this->db->join("receive_order","receive_order.no_receive = receive_item.no_receive","left");
		$this->db->where("receive_order.no_po",$no_po);
		$this->db->where("receive_item.sku",$sku);
	...
	function updateStatusHutang_potongretur($noPO,$potongan_retur){
		$update_hutang = array(
									"potongan_retur" 	=> $potongan_retur
							  );
	...
	function infoPembayaran($noPayment){
		$this->db->select(array("hutang_order.no_payment","supplier.supplier","hutang_order.tanggal_pembayaran","hutang_order.keterangan","hutang_order.no_po","users.first_name as nama_user","payment_type_debt.paymentType","hutang_order.pembayaran"));
		$this->db->from("hutang_order");
		$this->db->join("users","users.id = hutang_order.id_pic","left");
		$this->db->join("payment_type_debt","payment_type_debt.id = hutang_order.id_payment","left");
	...
	function potongan_retur($noTagihan){
		$this->db->select("potongan_retur");
		$this->db->from("hutang");
		$this->db->where("no_tagihan",$noTagihan);
		$query = $this->db->get()->row();
		return $query->potongan_retur;
	}
